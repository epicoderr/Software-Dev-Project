...
class TestPaymentCard(unittest.TestCase):
    ...
    def test_negatiivisen_rahan_laataaminen_ei_toimi(self):
        self.kortti.lataa_rahaa(-500)  
        self.assertEqual(self.kortti.saldo_euroina(), 10.0)

    def test_syo_edullisesti_tarkka_maara_kortissa(self):
        kortti = Maksukortti(250) 
        kortti.syo_edullisesti()
        self.assertEqual(kortti.saldo_euroina(), 0.0)

    def test_syo_maukkaasti_tarkka_maara_kortissa(self):
